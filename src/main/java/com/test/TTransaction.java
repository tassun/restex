...
public class TTransaction {
	...
	public void fetchResult(java.sql.ResultSet rs) throws Exception {
		...
		setJournalname(rs.getString("journalname"));
		setJournaltype(rs.getString("journaltype"));		
	}

	public java.util.List<TTransaction> find(java.sql.Connection connection) throws Exception {
		java.util.List<TTransaction> result = new java.util.ArrayList<>();
		StringBuilder sql = new StringBuilder();
		sql.append("select ttransaction.*,tjournal.journalname,tjournal.journaltype ");
		sql.append("from ttransaction ");
		sql.append("left join tjournal on tjournal.journalid = ttransaction.journalid ");
		sql.append("where ttransaction.transdate = ? ");
		sql.append("order by transno ");
		System.out.println(sql);
		System.out.println("transdate="+transdate);
		try(java.sql.PreparedStatement ps = connection.prepareStatement(sql.toString())) {
			ps.setDate(1,getTransdate());
	...
	public void obtain(java.util.Map map) {
		...
		if(transid!=null) setTransid(transid);
		String journalid = (String)map.get("journalid");
		if(journalid!=null) setJournalid(journalid);
		String remark = (String)map.get("remark");
		if(remark!=null) setRemark(remark);
		Number amount = (Number)map.get("amount");
		if(amount!=null) setAmount(new java.math.BigDecimal(amount.doubleValue()));
		String transdate = (String)map.get("transdate");
		if(transdate!=null) {
			setTransdate(java.sql.Date.valueOf(transdate));
		}
	}
	...
	public int retrieve(java.sql.Connection connection) throws Exception {
		...
		sql.append("from ttransaction ");
		sql.append("left join tjournal on tjournal.journalid = ttransaction.journalid ");
		sql.append("where ttransaction.transid = ? ");
		System.out.println(sql);
		try(java.sql.PreparedStatement ps = connection.prepareStatement(sql.toString())) {
			ps.setString(1,getTransid());
			try(java.sql.ResultSet rs = ps.executeQuery()) {
